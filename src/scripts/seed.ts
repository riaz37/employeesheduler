...
import { Employee, EmployeeRole, EmployeeStatus } from '../employees/schemas/employee.schema';
import { Shift, ShiftStatus, ShiftType } from '../shifts/schemas/shift.schema';
import { TimeOff, TimeOffType, TimeOffStatus, TimeOffPriority } from '../time-off/schemas/time-off.schema';
...
// Sample data
const LOCATIONS = [
  { name: 'Downtown Office', address: '123 Main St, Downtown', coordinates: [-73.935242, 40.730610] },
  { name: 'West Campus', address: '456 West Ave, Suburbs', coordinates: [-73.945242, 40.720610] },
  { name: 'East Warehouse', address: '789 East Blvd, Industrial', coordinates: [-73.925242, 40.740610] },
  { name: 'North Branch', address: '321 North Rd, Residential', coordinates: [-73.935242, 40.750610] },
];
...
const SHIFT_TEMPLATES = [
  { name: 'Morning Shift', startTime: '08:00', endTime: '16:00', type: ShiftType.REGULAR },
  { name: 'Afternoon Shift', startTime: '16:00', endTime: '00:00', type: ShiftType.REGULAR },
  { name: 'Night Shift', startTime: '00:00', endTime: '08:00', type: ShiftType.NIGHT },
  { name: 'Weekend Shift', startTime: '10:00', endTime: '18:00', type: ShiftType.WEEKEND },
  { name: 'Overtime Shift', startTime: '18:00', endTime: '22:00', type: ShiftType.OVERTIME },
];
...
async function createEmployees(): Promise<any[]> {
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    const availabilityWindows = generateAvailabilityWindows(role, isPartTime, isOvernight);
...
function generateSkillsForRole(role: EmployeeRole): any[] {
  const roleSkills = {
    [EmployeeRole.MANAGER]: ['Project Management', 'Communication', 'Data Analysis'],
    [EmployeeRole.SUPERVISOR]: ['Project Management', 'Communication', 'Data Analysis'],
    [EmployeeRole.STAFF]: ['JavaScript', 'Python', 'React', 'Node.js', 'MongoDB'],
    [EmployeeRole.SPECIALIST]: ['Data Analysis', 'Communication', 'Customer Support'],
...
function generateAvailabilityWindows(role: EmployeeRole, isPartTime: boolean, isOvernight: boolean): any[] {
  if (isPartTime) {
    return [
      {
        dayOfWeek: 1, // Monday
  ...
  for (let day = 0; day < 30; day++) {
    ...
    for (const location of LOCATIONS) {
      for (const team of TEAMS) {
        ...
        const teamShifts = generateTeamShifts(currentDate, location, team, employees);
...
function generateTeamShifts(date: Date, location: any, team: any, employees: any[]): any[] {
  const shifts = [];
  ...
  const requirements = determineTeamRequirements(team.name, teamEmployees.length);
  
  for (const requirement of requirements) {
    const template = SHIFT_TEMPLATES.find(t => t.name.includes(requirement.shiftType));
    if (!template) continue;
    
    // Find available employees for this role
    const availableEmployees = teamEmployees.filter(emp => 
      emp.role === requirement.role && emp.status === 'active'
    );
    
    // Assign employees (some shifts might be understaffed for realism)
    const assignedEmployees = availableEmployees.slice(0, Math.min(requirement.quantity, availableEmployees.length));
...
function determineTeamRequirements(teamName: string, employeeCount: number): any[] {
  const baseRequirements = {
    'Engineering': [
      { role: 'staff', quantity: Math.max(2, Math.floor(employeeCount * 0.6)) },
      { role: 'specialist', quantity: Math.max(1, Math.floor(employeeCount * 0.3)) },
      { role: 'supervisor', quantity: Math.max(1, Math.floor(employeeCount * 0.1)) },
    ],
    'Sales': [
      { role: 'staff', quantity: Math.max(2, Math.floor(employeeCount * 0.7)) },
      { role: 'manager', quantity: Math.max(1, Math.floor(employeeCount * 0.3)) },
    ],
    'Customer Service': [
      { role: 'staff', quantity: Math.max(3, Math.floor(employeeCount * 0.8)) },
      { role: 'supervisor', quantity: Math.max(1, Math.floor(employeeCount * 0.2)) },
    ],
    'Operations': [
      { role: 'manager', quantity: Math.max(1, Math.floor(employeeCount * 0.4)) },
      { role: 'specialist', quantity: Math.max(1, Math.floor(employeeCount * 0.3)) },
      { role: 'staff', quantity: Math.max(1, Math.floor(employeeCount * 0.3)) },
    ],
  };
  
  return baseRequirements[teamName] || [
...
async function createTimeOffRequests(employees: any[]): Promise<any[]> {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const timeOffType = Object.values(TimeOffType)[Math.floor(Math.random() * Object.values(TimeOffType).length)];
    const priority = Object.values(TimeOffPriority)[Math.floor(Math.random() * Object.values(TimeOffPriority).length)];
    ...
    const timeOffRequest = new TimeOff({
      ...
      approvalWorkflow: {
        ...
        approvedAt: status === TimeOffStatus.APPROVED ? new Date(requestDate.getTime() - 3 * 24 * 60 * 60 * 1000) : undefined,
        approvedBy: status === TimeOffStatus.APPROVED ? employees[0]._id : undefined,
        rejectedAt: status === TimeOffStatus.REJECTED ? new Date(requestDate.getTime() - 2 * 24 * 60 * 60 * 1000) : undefined,
        rejectedBy: status === TimeOffStatus.REJECTED ? employees[0]._id : undefined,
        rejectionReason: status === TimeOffStatus.REJECTED ? 'Insufficient coverage' : undefined,
      },
...
async function createSchedules(shifts: any[], employees: any[], timeOffRequests: any[]): Promise<any[]> {
  const schedules = [];
  const startDate = new Date();
  startDate.setDate(startDate.getDate() - 30);
  ...
  for (let day = 0; day < 30; day++) {
    ...
    const dayShifts = shifts.filter(shift => 
      shift.date.toDateString() === currentDate.toDateString()
    );
    
    // Get time-off requests for this date
    const dayTimeOff = timeOffRequests.filter(to => 
      to.startDate <= currentDate && to.endDate >= currentDate && to.status === 'approved'
    );
    ...
    const metrics = calculateMetrics(dayShifts, dayEmployees, coverage, conflicts);
    
    for (const location of LOCATIONS) {
      for (const team of TEAMS) {
        const teamShifts = dayShifts.filter(shift => 
...
function detectConflicts(shifts: any[], timeOffRequests: any[]): any[] {
  ...
  for (const shift of shifts) {
    for (const employeeId of shift.assignedEmployees) {
      if (!employeeShifts[employeeId.toString()]) employeeShifts[employeeId.toString()] = [];
      employeeShifts[employeeId.toString()].push(shift);
    }
  }
  
  // Detect overlapping shifts for same employee
  for (const [employeeId, employeeShiftsList] of Object.entries(employeeShifts)) {
...
function calculateMetrics(shifts: any[], employees: any[], coverage: any[], conflicts: any[]): any {
  return {
    totalShifts: shifts.length,
    totalEmployees: employees.length,
    totalHours: shifts.reduce((sum, shift) => sum + shift.totalHours, 0),
    averageUtilization: coverage.length > 0 ? coverage.reduce((sum, c) => sum + c.coverage, 0) / coverage.length : 0,
    coverageScore: coverage.length > 0 ? coverage.reduce((sum, c) => sum + c.coverage, 0) / coverage.length : 0,
    conflictCount: conflicts.length,
    criticalConflicts: conflicts.filter(c => c.severity === 'critical').length,
  };
}

async function displaySummary(employees: any[], shifts: any[], timeOffRequests: any[], schedules: any[]) {
  console.log('\n📊 SEEDING SUMMARY:');
  console.log('==================');
  ...
  console.log(`   - Staff: ${employees.filter(e => e.role === 'staff').length}`);
  console.log(`   - Specialists: ${employees.filter(e => e.role === 'specialist').length}`);
  console.log(`   - Trainees: ${employees.filter(e => e.role === 'trainee').length}`);
  console.log(`   - Part-time: ${employees.filter(e => e.workPreference.maxHoursPerWeek === 20).length}`);
  console.log(`   - Overnight workers: ${employees.filter(e => e.workPreference.preferredShifts.includes('night')).length}`);
  
  console.log(`⏰ Shifts: ${shifts.length}`);
  console.log(`   - Regular: ${shifts.filter(s => s.type === 'regular').length}`);
  console.log(`   - Night: ${shifts.filter(s => s.type === 'night').length}`);
  console.log(`   - Weekend: ${shifts.filter(s => s.type === 'weekend').length}`);
  console.log(`   - Overtime: ${shifts.filter(s => s.type === 'overtime').length}`);
  
  console.log(`🏖️ Time-off Requests: ${timeOffRequests.length}`);
  console.log(`   - Approved: ${timeOffRequests.filter(t => t.status === 'approved').length}`);
  console.log(`   - Pending: ${timeOffRequests.filter(t => t.status === 'pending').length}`);
  console.log(`   - Rejected: ${timeOffRequests.filter(t => t.status === 'rejected').length}`);
  
  console.log(`📅 Schedules: ${schedules.length}`);
  console.log(`   - Draft: ${schedules.filter(s => s.status === 'draft').length}`);
  console.log(`   - Published: ${schedules.filter(s => s.status === 'published').length}`);

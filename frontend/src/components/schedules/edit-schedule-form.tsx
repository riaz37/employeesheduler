...
import { updateScheduleSchema, type UpdateScheduleFormData } from '@/lib/validations/schedule';
...
export function EditScheduleForm({ schedule, onSuccess }: EditScheduleFormProps) {
  ...
  const onSubmit = async (data: UpdateScheduleFormData) => {
    try {
      await updateScheduleMutation.mutateAsync({ id: schedule._id, data });
      onSuccess();
    } catch (error) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="SCHED001" {...field} disabled />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        {/* Shifts */}
        <FormSection title="Shifts" description="Manage shifts for this schedule">
